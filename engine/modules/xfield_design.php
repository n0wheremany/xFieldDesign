...
skype: gameerblog
=============================================================================
Отдельное спасибо за помощь : проверку, форматирование и правку кода ПафНутиЙ http://pafnuty.name
=============================================================================
Файл:  xfield_design.php
=============================================================================
Версия модуля : 0.02b Alpha Release
=============================================================================
/*
 * Что может:
 * - Оформлять доп поля ╰(▔∀▔)╯
...
for ($j = 0; $j < count($xfieldsdata); $j++) { // цикл...
    ...
    if (stripos($tpl->copy_template, "[xf:") !== false) { // а тут совсем дела плохие
        $tpl->copy_template = preg_replace_callback("#\\[(xf):(.+?)\\](.*?)\\[/xf\\]#is",
            // самая что ни есть анонимная функция
    ...
    if ($xfieldsdata[$j][3] == "text" and $flag_desgn_xf) {
        ...
        if ($val_xfield[$xfieldsdata[$j][0]]) { // берем значение если оно есть в противном случае берем если есть по умолчанию а иначе пусто
